...
function TrinketMenu.GetID(bag,slot)
	local id
	if slot then
		null,null,id = string.find(GetContainerItemLink(bag,slot) or "","item:(%d+)")
	else
		null,null,id = string.find(GetInventoryItemLink("player",bag) or "","item:(%d+)")
	end
	return id
end
...
function TrinketMenu.GetNameByID(id)
	if id==0 then
		...
		local name,null,quality,null,null,null,null,null,null,texture = GetItemInfo(id or "")
...
function TrinketMenu.PopulateSort(which)
	...
	for i=0,4 do
		for j=1,GetContainerNumSlots(i) do
			...
			null,null,null,null,null,null,null,null,equipLoc = GetItemInfo(id or "")
...
function TrinketMenu.SortTooltip(self)
	...
	null,null,itemLink = string.find(itemLink or "","(item:%d+:%d+:%d+:%d+:%d+:%d+:%d+)")
...
function TrinketMenu.CanCooldown(inv)
	local null,null,enable = GetInventoryItemCooldown("player",inv)
	return enable==1
end
...
function TrinketMenu.ProcessAutoQueue(which)
	...
	local null,null,id,name = string.find(GetInventoryItemLink("player",13+which) or "","item:(%d+).+%[(.+)%]")
			...
			if not ready or enable==0 or (TrinketMenuQueue.Stats[list[i]] and TrinketMenuQueue.Stats[list[i]].priority) then
				if TrinketMenu.TrinketNearReady(list[i]) then
					if GetItemCount(list[i])>0 and not IsEquippedItem(list[i]) then
						null,bag,slot = TrinketMenu.FindItem(list[i])
						if bag then
